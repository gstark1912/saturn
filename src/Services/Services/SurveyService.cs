...
namespace Services
{
    public class SurveyService
    {
        ...
        {
            //si existen productos no elimino
            var surveyCompletionParents = this.modelContext
                .SurveyCompletionParent
                .Where(x => x.Category.Id == categoryId).ToList();

            if (surveyCompletionParents.Count > 0) return;

            RemoveAllChildrenCategories(categoryId);
            var category = this.modelContext
                .Categories
        ...
        private void RemoveAllChildrenCategories(int catID) {
            List<Category> categories = this.modelContext
                .Categories
                .Where(x => x.parentCategory.Id == catID).ToList();
